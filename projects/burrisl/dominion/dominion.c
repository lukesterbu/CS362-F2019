        ...
        while(card_not_discarded) {
            if (state->hand[currentPlayer][selectedCard] == estate) { //Found an estate card!
                ...
                if (supplyCount(estate, state) > 0) {
                    gainCard(estate, state, 0, currentPlayer); // This also decrements the supplyCount - BUG?
                    state->supplyCount[estate]--; //Decrement estates again?
                    if (supplyCount(estate, state) == 0) {
                        isGameOver(state);
                    }
                }
...
void doAmbassador(int currentPlayer, int choice1, int choice2, struct gameState *state, int handPos)
{
    ...
    for (int player = 0; player < state->numPlayers; player++)
    {
        if (player != currentPlayer)
        {
            gainCard(state->hand[currentPlayer][choice1], state, 0, player);
        }
    }
    //discard played card from hand
    discardCard(handPos, currentPlayer, state, 0);
...
void doMine(int currentPlayer, int choice1, int choice2, struct gameState *state, int handPos)
{
    ...
    gainCard(choice2, state, 1, currentPlayer); // Should be a 2 not a 1 **BUG**
    //discard card from hand
    discardCard(handPos, currentPlayer, state, 0);
    //discard trashed card
    for (int card = 0; card < state->handCount[currentPlayer]; card++)
